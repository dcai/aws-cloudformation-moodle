...
$thispageurl = new moodle_url('/spark/admission/application_edit.php', array('id' => $id, 'action' => $action));
...
if ($action == 'edit') {
    if (has_capability('spark/admission:processapplication', $contextsystem, null, false)) {
        $toform = $DB->get_record('spark_admission_app', array('id' => $id, 'approved' => 1, 'deleted' => 0), '*', MUST_EXIST);
        $toform->dataclerk = 1;
    } else {
        $toform = $DB->get_record('spark_admission_app', array('id' => $id, 'deleted' => 0), '*', MUST_EXIST);
    }
    $mform = new student_application_form(null, (array)$toform);
} else {
    $mform = new student_application_form(null, array());
}
...
if ($mform->is_cancelled()) {
    ...
    if ($action == 'add') {
        ...
        $fromform->id = $DB->insert_record('spark_admission_app', $fromform);
        redirect(new moodle_url('/spark/admission/application.php'), get_string('successful', 'spark_admission'), 0);
    } else {
        $fromform->id = $id;
        $fromform->timemodified = time();
        $DB->update_record('spark_admission_app', $fromform);
        redirect(new moodle_url('/spark/admission/application.php'), get_string('successful', 'spark_admission'), 0);
    }
    exit;
}
